...
|---|---|---|---|
| **4.0.0** | 2026-05-18 | Antigravity | **Hoàn thiện Bộ UI Kit Hoàn chỉnh**: <br>1. Thiết kế lại hoàn toàn bảng màu Dark Mode sang tông Slate/Graphite sâu thẳm, hiện đại.<br>2. Bổ sung nút **Theme Switcher** (chuyển đổi Light/Dark) trên thanh Header của Popup, tự động lưu thông qua `StatePersistence`.<br>3. Thêm 9 component mới cấp cao: `MNImage` (hiển thị ảnh, tích hợp Skeleton), `MNTabs` (bố cục chia trang), `MNTextArea` (nhập liệu nhiều dòng, auto-resize), `MNProgressBar` & `MNSpinner` (hiển thị trạng thái xử lý/tải), `MNTooltip` (nhãn giải thích), `MNRadioGroup` (bộ chọn đơn), `MNTable` (bảng dữ liệu), và `MNDialog` (hộp thoại xác nhận).<br>4. Cập nhật Typescript (`mngui.d.ts`) hỗ trợ IntelliSense tối đa. |
